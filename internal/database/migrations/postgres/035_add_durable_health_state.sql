...
CREATE TABLE health_file_revisions (
    ...
    file_health_id BIGINT NOT NULL REFERENCES file_health(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX idx_health_file_revisions_active
    ON health_file_revisions(file_health_id) WHERE active = TRUE;
...
CREATE TABLE health_providers (
    ...
    tombstoned_at TIMESTAMPTZ DEFAULT NULL,
    created_at TIMESTAMPTZ NOT NULL,
...
CREATE TABLE health_run_chunks (
    ...
    segment_count BIGINT NOT NULL CHECK(segment_count > 0),
    tested_bitmap BYTEA NOT NULL,
    present_bitmap BYTEA NOT NULL,
    absent_bitmap BYTEA NOT NULL,
    corrupt_bitmap BYTEA NOT NULL,
    temporary_bitmap BYTEA NOT NULL,
    inconclusive_bitmap BYTEA NOT NULL,
...
CREATE FUNCTION health_clear_durable_state_before_file_delete()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
FOR EACH ROW
EXECUTE FUNCTION health_clear_durable_state_before_file_delete();
...
DROP TRIGGER IF EXISTS health_clear_durable_state_before_file_delete ON file_health;
DROP FUNCTION IF EXISTS health_clear_durable_state_before_file_delete();
